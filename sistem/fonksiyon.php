<?php
    ...
    function klasor($dizin){
        ...
        while($dosya = readdir($dizinAc)){
            ...
            echo "<option ".(tema_dir==$dosya? 'selected' : null)." value='".$dosya."'>".$dosya."</option>";
        }
        }
    }
    function timeConvert($date){
        $seconds = (7200+time()-strtotime($date));
        $hours = floor($seconds/3600);
        $minutes = floor(($seconds/60) % 60);
        $seconds = $seconds % 60;
        if($hours>0) return $hours." saat ".$minutes." dakika ".$seconds." saniye ";
        else if($minutes>0) return $minutes." dakika ".$seconds." saniye önce";
        else  return $seconds." saniye önce";
    }
